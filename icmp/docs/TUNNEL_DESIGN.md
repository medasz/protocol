...
为了在无连接的 ICMP 协议中实现 TCP 级别的功能，我们必须在每次发送的 ICMP Payload 开头插入一段 14 字节的自定义协议头。
...
type TunnelHeader struct {
	...
	Seq       uint32 // 序列号 (表示本包发送的第一个字节的序号)
	Ack       uint32 // 确认号 (表示期望对方发送的下一个字节的序号)
	Length    uint16 // 实际数据的长度 (不包含本头部的 14 字节)
}
```
